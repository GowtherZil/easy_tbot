    ...
    def __getattribute__(self, name):
        ...
        try:
            ...
            if isinstance(self.__settings, dict):
                return self.__settings[name] # Try to return settings attribute if is a dictionary
            else:
                return getattr(self.__settings,name) # Try to return settings attribute to generic type
    ...
    def setup_shards(self, configurator:Callable[..., Any]):
        ...
        for shard in self.SHARDS: # itterate over shards
            try:
                ...
                setup.log(logging.WARNING, str(e)) # This must be informed as a warning and can be ignored 
            except Exception as e: 
                setup.log(logging.ERROR, str(e)) # No other type of exception is expected, therefore they are reported as an error
