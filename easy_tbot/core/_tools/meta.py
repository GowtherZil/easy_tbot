            ...
            def __new__(cls, name, this_bases, d):
                if sys.version_info[:2] >= (3, 7):  # whe check for python version
                    ...
                    resolved_bases = types.resolve_bases(
                        bases
                    )  # some __build_class__ mimic already promised
                ...
                return meta_mix(
                    name, resolved_bases, d
                )  # return the metaclass mixing with updated class bases

            @classmethod
            def __prepare__(
                cls, name, this_bases
            ):  # overload this to prepare metaclass as is espected
                return meta_mix.__prepare__(name, bases)

        return type.__new__(
            metaclass, "temporary_class", (), {}
        )  # return the dummy class that is now a super charged dummy class
